...
const UpdateAccreditationForm: React.FC = () => {
  ...
  const removeMember = async (index: number) => {
    ...
    if (memberId) {
      try {
        await axios.delete(`http://localhost:8800/members-accred/${memberId}`);
      } catch (error) {
  ...
  const removeOfficer = async (index: number) => {
    ...
    if (officerId) {
      try {
        await axios.delete(`http://localhost:8800/members-accred/${officerId}`);
      } catch (error) {
  ...
  const removeActivity = async (index: number) => {
    ...
    if (activityId) {
      try {
        await axios.delete(`http://localhost:8800/activities/${activityId}`);
      } catch (error) {
      ...
      <div className="faciform-content">
        <div className="incampus-content">
          <div className="incampus-header">
            ...
            <h1 className="incampus-title">Update Accreditation Application Form</h1>
          </div>
          ...
          <form id="accreditationForm" className="accre-form-content-form" onSubmit={handleSubmit}>
            <div className="accre-form-content-grid">
              <div className="accre-form-content-section">
                ...
                <input type="file" name="constitution" accept="application/pdf" className="accre-form-content-input" onChange={(e) => handleFileChange(e, setConstitutionFileName,  'constitution')} />
                {constitutionFileName && <p>Current File: {constitutionFileName}</p>}
              </div>
              <div className="accre-form-content-section">
            ...
            <div className="accre-form-content-section">
              <label className="accre-form-content-label">5. Adviser's Letter of Acceptance</label>
...
export default UpdateAccreditationForm;