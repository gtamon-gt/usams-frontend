...
const DirectorApproval: React.FC = () => {
      ...
      try {
        ...
        setProposals(proposalsData.filter((proposal: Proposal) =>
          ...
          proposal.on_revision === 0 && (
            (proposal.pros_type === 'In Campus' && proposal.pros_nature === 'Co-Curricular' && proposal.step_at === 3) ||
            (proposal.pros_type === 'In Campus' && proposal.pros_nature === 'Extra-Curricular' && proposal.step_at === 2) ||
            (proposal.pros_type === 'Off-Campus A' && proposal.step_at === 3) ||
            (proposal.pros_type === 'Off-Campus B' && proposal.step_at === 3)
          )
        ));
  ...
  const handleApprove = async (pros_key: string) => {
    try {
      await axios.put(`http://localhost:8800/proposals/approve/osa/${pros_key}`);
    ...
    if (selectedProposal && comment) {
      try {
        await axios.post(`http://localhost:8800/proposals/reject/osa/${selectedProposal.pros_key}`, {
          comment: `[${director?.dir_name}] ${comment}`
        });
