...
const AdviserApproval: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const foundAdviser = advisers.find((adv: Adviser) => adv.user_id === userId);
        if (foundAdviser) {
          setAdviser(foundAdviser);

          // find organizations advised by the adviser
          const advisedOrganizations = organizationsData.filter((org: Organization) => org.adv_id === foundAdviser.adv_id);
  ...
  const handleApprove = async (pros_key: string) => {
    try {
      await axios.put(`http://localhost:8800/proposals/approve/adviser/${pros_key}`);
      setProposals(proposals.map(proposal => proposal.pros_key === pros_key ? { ...proposal, status: 'Approved by Organization Adviser' } : proposal));
  ...
  const handleSubmitReject = async () => {
    if (selectedProposal && comment) {
      try {
        await axios.post(`http://localhost:8800/proposals/reject/adviser/${selectedProposal.pros_key}`, {
          comment: `[${adviser?.adv_name}] ${comment}`
        });
        setProposals(proposals.map(proposal => proposal.pros_key === selectedProposal.pros_key ? { ...proposal, status: 'Rejected by Organization Adviser' } : proposal));
...
export default AdviserApproval;