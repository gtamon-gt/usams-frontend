 ...
 const handleReject = (books) => {
  ...
  const handleSubmitReject = async (e) => {
    ...
    if (selectedProposal && comment) {
        try { 
            const response = await axios.post(`http://localhost:8800/dsareturned/${selectedProposal.reps_id}`, {
...
const handleApprove = (reps_id) =>{
    try{ 
   // e.preventDefault() 
     axios.put('http://localhost:8800/dsaapproved/'+reps_id);
      alert('Form Approved!');
        ...
        <div key={list.reps_id} className="proposal-row">
          ...
          <div className="proposal-cell ext">
            <button  onClick={() => {
                   ...
                   navigate(`/approvereportsform/${list.reps_id}`, { state: { userId, userInfo } });
