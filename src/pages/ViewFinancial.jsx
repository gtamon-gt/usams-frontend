    ...
    useEffect(() => {
      const fetchAllBooks = async () => {
        try {
          ...
          // 1️⃣ Filter records by matching orgId
          const filteredProposals = res.data.filter((record) => record.org_id === orgId);
    
          // 2️⃣ Ensure unique `group_id` values
          const uniqueBooks = [];
          const seenGroupIds = new Set();
    
          filteredProposals.forEach((record) => {
          ...
          // 3️⃣ Set state with unique filtered records
          setBooks(uniqueBooks);
          console.log("Filtered Unique Books:", uniqueBooks);

          const commentsRes = await axios.get("http://localhost:8800/commentsfinancial");
          const commentsData = commentsRes.data;
  
          // Map comments to their corresponding proposals
          const commentsMap = {};
          filteredProposals.forEach((list) => {
            commentsMap[list.fr_id] = commentsData.filter(
              (comment) => comment.fr_id === list.fr_id
            );
          });
...
<div className="adviser-approval-content">
  <div className="title-top-part">
    <h2>Submitted Financial Reports</h2>
          ...
          <div className="proposal-cell ext">
            ...
            <button 
              onClick={() => {
                              ...
                              navigate(`/updatefinancial/${orgId}/${list.group_id}`, { state: { userId, userInfo } });
            ...
            <button
                        ...
                        onClick={() => toggleComments(list.fr_id)}
                      >
                        {showComments[list.fr_id] ? 'Hide Comments' : 'Show Comments'}
              </button>
          
          
          </div>
         
          {showComments[list.fr_id] && (
                    <tr>
                      <td colSpan={5}>
                        <div className="comments-section">
                          {comments[list.fr_id] && comments[list.fr_id].length > 0 ? (
                            comments[list.fr_id].map((comment) => (
