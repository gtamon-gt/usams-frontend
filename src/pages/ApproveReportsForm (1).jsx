...
const ApproveReportsForm = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:8800/financial_statements/fetch/${reps_id}`)
      .then((res) => {
        console.log("Fetched Data:", res.data);
          ...
          <div>
            ...
            <div className="detailsform1">
              <div className='col1details'>
                ...
                <div className='financialdocstext1'>Less: Expenses </div>
                ...
                <br></br>
                <br></br>
                <div className='financialdocstext1'>Financial Statements: </div>
...
<table className="financial-table">
  ...
  <tbody>
    {fetchedData2.length > 0 ? (
      fetchedData2.map((row2, index) => (
        <tr key={index}>
          <td>{row2.date}</td>
          <td>{row2.reference}</td>
          <td>{row2.recipient}</td>
          <td>{row2.amount2}</td>
        </tr>
      ))
    ) : (
                            ...
                            imgSet.financialfiles1.map((financialfiles1, imgIndex) => (
                              <div key={`financialfiles-${index}-${imgIndex}`}>
                                <img
                                  ...
                                  onClick={() => handleImageClick2(`http://localhost:8800/uploads/${financialfiles1}`)}
                                />
                              </div>
                            ))}
                        </div>
                            ...
                            {imgSet.documentations &&
                              imgSet.documentations.map((documentations, imgIndex) => (
                                <div key={`documentations-${index}-${imgIndex}`}>
                                  <img
                                    src={`http://localhost:8800/uploads/${documentations}`}
                                    alt={`Fetched Image ${imgIndex + 1}`}
                                    style={{
                                      width: "200px",
                                      margin: "10px 0",
